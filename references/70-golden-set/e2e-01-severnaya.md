...
Сама стратегия — в папке [`e2e-01-severnaya/`](e2e-01-severnaya/): восемь страниц вики (`index` + пять разделов + два приложения) и сборка в один переносимый файл [`strategy.html`](e2e-01-severnaya/strategy.html). Собирается скриптом `build-html.py` без зависимостей.
